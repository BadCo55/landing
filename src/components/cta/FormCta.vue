<template>
  ...
  >
    ...
    <div class="relative z-10 text-white w-full md:w-1/2 md:pr-10 text-left mb-10 md:mb-0 mt-10">
      <img
        :src="appStore.logoDark"
        alt=""
        class="mx-auto lg:mx-0 w-[20rem] mb-10 bg-slate-900/40 p-3 rounded-xl"
      />
    ...
    <div class="relative z-10 bg-white rounded-lg shadow-lg p-6 md:p-10 w-full md:w-1/2">
      <div class="flex flex-col lg:flex-row items-center gap-5 mb-5">
        ...
        <Button
          label="OR, Click Here to Get a FREE Quote!"
          class="!text-white !font-bold !text-xl"
          raised
          severity="success"
          @click="requestQuoteClick('request_quote_click', 'cta', 'form_cta')"
        />
      </div>
      <form class="space-y-6" @submit.prevent="submitForm">
        ...
        <div class="flex flex-col md:flex-row gap-6">
          <!-- Email Field -->
          <div class="flex-1">
            <label for="email" class="block text-sm font-medium text-gray-700 flex-wrap"
              >Email Address</label
            >
            <input
        ...
        <!-- Submit Button -->
        <Button type="submit" class="w-full !text-white"> Send Message </Button>
      </form>
    </div>
  </div>
</template>
